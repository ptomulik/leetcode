...
having arc capacities $r^{\eta}_{ij} = u_{ij} - x^{\eta}_{ij}$.}. We treat the
...
= z(r^{\eta})$. The actual algorithm does not maintain $x$, as we are only
interested in $r$. It's, thus, just enough to keep track of $r$, $e$, $D$ and
$E$ only.

\paragraph{Additional requirement -- strong connectivity.}

Note, that the above algorithm is not applicable to an arbitrary network $G,
u$. We can easily find examples, where it yields invalid results for
a~legitimate input data. One such example is given on figure~\ref{fig:XUM0T},
where, according to the algorithm, shortest paths are processed in order
$P_{30}$, $P_{42}$ ($|P_{30}| = 2$, $|P_{42}| = 3$). The result from this
iteration is $z(r^2) = 0$, while it should be $z(r^{*}) = 3$ (consider choosing
paths $P_{32}$ and $P_{40}$, both of length $P_{kl} = 1$).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
  \centering
  \begin{subfigure}[t]{0.33\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth]
      \graph[
        ...
        1 <-["1"] 0[red] <-["1"] 4,
        2 <-["1",very thick,blue] 3[blue],
        1 ->["1"'] 2 ->["1",very thick,blue]0,
      };
      \begin{scope}[node distance=0mm,font={\footnotesize}]
        % node balances
        \node [above=of 0] {+1};
        \node [above=of 4] {-1};
        \node [below=of 2] {+1};
        \node [below=of 3] {-1};
        \node [above=of 1] {0};
        \node at ($(0)!0.5!(3)$) {$\eta=0$};
      \end{scope}
    \end{tikzpicture}
    \caption{$D=\left\lbrace 3,4 \right\rbrace$,
             $E=\left\lbrace 0,2 \right\rbrace$, \\
             $(k,l)=(3,0)$, $|P_{30}| = 2$, \\
             $z(r^0) = 5$}
  \end{subfigure}
  \begin{subfigure}[t]{0.33\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth]
      \graph[
        ...
        1 <-["1",very thick,blue] 0 <-["1",very thick,blue] 4[blue],
        2[red] <-[dashed,gray,"0"] 3,
        1 ->["1"',very thick,blue] 2 ->[dashed,gray,"0"]0,
      };
      \begin{scope}[node distance=0mm,font={\footnotesize}]
        % node balances
        \node [above=of 0] {0};
        ...
        \node [below=of 3] {0};
        \node [above=of 1] {0};
        \node at ($(0)!0.5!(3)$) {$\eta=1$};
      \end{scope}
    \end{tikzpicture}
    \caption{$D=\left\lbrace 4 \right\rbrace$,
             $E=\left\lbrace 2 \right\rbrace$, \\
             $(k,l)=(4,l)$, $|P_{42}|=3$ \\
             $z(r^1) = 3$}
  \end{subfigure}
  \begin{subfigure}[t]{0.32\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth]
      ...
      \begin{scope}[node distance=0mm,font={\footnotesize}]
        ...
        \node [below=of 3] {0};
        \node [above=of 1] {0};
        \node at ($(0)!0.5!(3)$) {$\eta=2$};
      \end{scope}
    \end{tikzpicture}
    \caption{$D=\left\lbrace \right\rbrace$,
             $E=\left\lbrace \right\rbrace$, \\
             $z(r^2) = 0$}
  \end{subfigure}
  \caption{An example iteration yielding incorrect result $z(r^2) = 0$ instead of $3$.}
  \label{fig:XUM0T}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

One may think, that this is just a matter of picking the shortest paths
$P_{kl}$ in appropriate order, e.g. selecting the paths $P_{kl}: (k,l) \in D
\times E$ sorted by length (shortest first). While the claim ``picking paths in
appropriate order'' may have sense, determining the ``appropriate order'' is
hard if not impossible. Sorting the shortest paths by their length would not
guarantee correctness. It happens, for example, that multiple paths of same
length compete for certain units of flow, and choosing one over the other may
yield different results. This is depicted on figure~\ref{fig:CT5N5}. Initially,
there are three possible paths from deficit to excess nodes, all of length $2$
-- two alternative paths from node $3$ to $1$ ($3-0-1$ and $3-4-1$) and one
path from node $3$ to $2$ ($3-4-2$). Assume that the algorithm selected the
pair $(k,l) = (3,1)$, for which Dijkstra's algorithm provided $P_{31} = 3-4-1$
as shortest path. Using it at iteration $\eta=0$ excludes the paths $3-4-2$ and
$3-0-1$ from further iterations. For the next iteration, a~new set of shortest
paths is evaluated consisting of a single path $3-0-1-2$ of length $3$. After
removing its flow, we reach network balance, but the circulation is $z(r^2) = 2$
instead of $z(r^{*})=3$ (using paths $P_{31} = 3-0-1$ and then $P_{32} = 3-4-2$
would lead to $z(r^2)=3$, a~circular along the single cycle $1-2-4-1$).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
  \centering
  \begin{subfigure}[t]{0.33\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth]
      \graph[
        ...
        0 <-["1"] 3[blue],
        1[red] <-["1",very thick,blue] 4,
        2,
        1 ->["1"'] 2, 2 ->["1"] 4 ->["1", bend left] 2,
        0 ->["1"'] 1, 3 ->["1",very thick,blue] 4,
      };
      \begin{scope}[node distance=0mm,font={\footnotesize}]
        % node balances
        \node [right=of 4] {0};
        ...
        \node [right=of 3] {-2};
        \node [left=of  0] {0};
        \node at ($(1)!0.5!(3)$) {$\eta=0$};
      \end{scope}
    \end{tikzpicture}
    \caption{$D=\left\lbrace 3 \right\rbrace$,
  ...
  \begin{subfigure}[t]{0.33\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth]
      \graph[
        ...
        0 <-["1",very thick,blue] 3[blue],
        1 <-["0",gray,dashed] 4,
        2[red],
        1 ->["1"',very thick,blue] 2, 2 ->["1"] 4 ->["1", bend left] 2,
        0 ->["1"',very thick,blue] 1, 3 ->["0",gray,dashed] 4,
      };
      \begin{scope}[node distance=0mm,font={\footnotesize}]
        % node balances
        \node [right=of 4] {0};
  ...
  \begin{subfigure}[t]{0.32\textwidth}
    ...
    \begin{tikzpicture}[>=Stealth]
      \graph[
        ...
        1 <-["0",gray,dashed] 4,
        2,
        1 ->["0"',gray,dashed] 2, 2 ->["1"] 4 ->["1", bend left] 2,
        0 ->["0"',gray,dashed] 1, 3 ->["0",gray,dashed] 4,
      };
      \begin{scope}[node distance=0mm,font={\footnotesize}]
        % node balances
        \node [right=of 4] {0};
    ...
    \caption{$D=\left\lbrace \right\rbrace$,
             ...
             $z(r^2) = 2$}
  \end{subfigure}
  \caption{An example iteration yielding incorrect result $z(r^2) = 2$ instead of $3$.}
  \label{fig:CT5N5}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Correctness of the above algorithm may be proved for certain class of networks.
First, observe that, by definition, the maximum circulation $z(r^{*})$ is
independent on arcs that do not participate in any cycles -- we can just remove
these arcs. These are arcs $(3,2)$ and $(4,0)$ on figure~\ref{fig:XUM0T}, or
arcs $(3,0)$, $(0,1)$, and $(3,4)$ on figure~\ref{fig:CT5N5}. In general, arcs
not participating in any cycles form so-called bridges between so-called
strongly connected components\footnote{A directed graph is strongly connected
if there is a path in each direction between each pair of nodes.}. At a~high
level we can draw a~condensation graph of $G$, where nodes represent strongly
connected components (SCCs) found in $G$ and these SCCs are connected by
bridges. This condensation graph is always acyclic, which means -- bridges
can not participate in the circulation (the entire circulation is enclosed
within SCCs). The iteration may therefore be ran on a~set of isolated SCCs
found in $G$ and the resultant circulation $z(r^{*})$ shall still be valid.
